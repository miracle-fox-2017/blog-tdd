...
describe('Testing blogs', function(){
  ...
  //get from id
  it('Article show test', function(done){
    chai.request(app)
    .get('/api/blog/'+idPost)
    // .set('token', token)
    .end(function(req, res){
  ...
  it('Article delete test', function(done){
    chai.request(app)
    .delete('/api/blog/'+idPost)
    .set('token', token)
    .end(function(req, res){
      expect(res).to.have.status(200)
      //code here
      expect(res.body.msg).to.equal('success')
      expect(res.body.deleted.title).to.equal(dummyTitle)
      expect(res.body.deleted.article).to.equal(dummyArticle)
